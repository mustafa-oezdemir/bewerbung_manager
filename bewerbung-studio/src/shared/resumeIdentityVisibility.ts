...
export const getResumeIdentityVisibilityCss = (
  ...
  if (hidden("heading")) {
    rules.push(
      `${scope} header:not([class*="section-heading"]) :is(h1,h2,[class*="__name"],[class*="__title"],[class*="__profession"],[class*="__kicker"],.kicker),${scope} .tabellarisch-pdf-continuation{display:none!important}`,
    );
  }
