...
import { externalUrl, formatPhoneForDisplay, formatUrlForDisplay } from "./contactPresentation";
...
const icon = (kind: keyof typeof iconPaths) => `<svg data-contact-icon="${kind}" viewBox="0 0 24 24" aria-hidden="true">${iconPaths[kind]}</svg>`;
const escape = (value: string) => value.replace(/[&<>"']/g, (char) => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;", "'": "&#39;" })[char]!);

export const getPehlioneContacts = (profile?: ApplicantProfile) => {
  const visible = { ...defaultResumePersonalFieldVisibility, ...profile?.resumePersonalFieldVisibility };
  return [
    { key: "location" as const, label: "Ort", visible: visible.address, value: [profile?.city, profile?.country].filter(Boolean).join(", "), href: "" },
    { key: "phone" as const, label: "Telefon", visible: visible.phone, value: formatPhoneForDisplay(profile?.phone), href: profile?.phone ? `tel:${profile.phone.replace(/[^\d+]/g, "")}` : "" },
    { key: "email" as const, label: "E-Mail", visible: visible.email, value: profile?.email || "", href: profile?.email ? `mailto:${profile.email}` : "" },
    { key: "linkedin" as const, label: "LinkedIn", visible: visible.linkedin, value: formatUrlForDisplay(profile?.linkedin || ""), href: externalUrl(profile?.linkedin || "") },
    { key: "github" as const, label: "GitHub", visible: visible.github, value: formatUrlForDisplay(profile?.github || ""), href: externalUrl(profile?.github || "") },
    { key: "website" as const, label: "Website", visible: visible.website, value: formatUrlForDisplay(profile?.portfolio || ""), href: externalUrl(profile?.portfolio || "") },
