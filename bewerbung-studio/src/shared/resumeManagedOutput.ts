...
const aliases: Record<string, string[]> = {
  summary: ["Zusammenfassung", "Kurzprofil", "Profil"], strengths: ["Stärken", "Kernkompetenzen"],
  experience: ["Berufserfahrung", "Erfahrung", "Beruflicher Werdegang"], education: ["Ausbildung", "Bildungsweg"],
  knowledge: ["Kenntnisse", "Fähigkeiten", "Technische Schwerpunkte", "Besondere Kenntnisse"],
  certifications: ["Zertifikate", "Weiterbildungen", "Weiterbildungen (Auswahl)", "Erfolge"], languages: ["Sprachen"], projects: ["Projekt-Highlight"],
};
const normalize = (value: string) => value.replace(/\s*·\s*Fortsetzung/i, "").trim().toLocaleLowerCase("de-DE");
...
export const applyManagedResumeOutput = (html: string, profile: ApplicantProfile | undefined, templateId: string, pageNumber = 1, totalPages = 1) => {
  ...
  roots.forEach((root, rootIndex) => {
    const enabled = (id: string) => entries.find((entry) => entry.id === id)?.visible !== false;
    if (!enabled("photo")) {
      const source = getProfileMediaSource(profile.photoPath);
      root.querySelectorAll("img").forEach((img) => {
        if (img.getAttribute("src") === source || /photo|foto/i.test(img.className)) {
          const frame = img.closest('[class*="__photo"],[class*="-header__photo"],[class*="-header-photo"]');
          (frame ?? img).remove();
        }
      });
    }
    ...
    for (const node of sectionNodes) {
      ...
      const entry = entries.find((entry) => entry.id.startsWith("special:") && elementId === `special.${entry.id.slice(8)}`)
        ?? entries.find((entry) => !entry.fixed && normalize(entry.title) === title)
        ?? entries.find((entry) => (aliases[entry.id] ?? []).some((alias) => normalize(alias) === title))
        ?? entries.find((entry) => !entry.fixed && elementId.endsWith(`.${entry.id}`));
      if (entry) {
    ...
    const main = root.querySelector(".pehlione-main,.pehlione-pdf-main,.elegant-main,.elegant-pdf-main,.modern-resume-left-column,.modern-pdf-left,.zweispaltig-main,.zweispaltig-pdf-main,.zeitgenoessisch-main,.zeit-pdf-main,.kreativ-main,.kreativ-pdf-main,.gepflegt-main,.gepflegt-pdf-main,.kompakt-left,.kompakt-pdf-columns>main,main")
      ...
      ?? root.querySelector(".page-content") ?? root;
    ...
    const container = (entry: ManagerSection) => entry.zone === "sidebar" ? sidebar : main;
    for (const entry of entries.filter((item) => !item.fixed)) {
      ...
      if (!entry.visible) { existing.forEach((node) => node.remove()); nodes.delete(entry.id); continue; }
      ...
      if (items.length && last) {
        ...
        content = group?.rendererType === "tag-list" ? `<div class="managed-tags">${itemHtml.map((item) => `<span>${item}</span>`).join("")}</div>`
          : ["two-column-list", "compact-grid"].includes(group?.rendererType ?? "") ? `<div class="managed-columns">${itemHtml.map((item) => `<div>${item}</div>`).join("")}</div>`
          : group?.rendererType === "text-list" ? itemHtml.map((item) => `<p>${item}</p>`).join("")
        ...
        content = special?.entries.map((item) => `<article><strong>${escape(item.title)}</strong><p>${[item.subtitle,item.location,item.date || [item.from,item.to].filter(Boolean).join(" – ")].filter(Boolean).map(escape).join(" · ")}</p><p>${escape(item.description)}</p>${item.bullets.length ? `<ul>${item.bullets.map((bullet) => `<li>${escape(bullet)}</li>`).join("")}</ul>` : ""}${item.url ? `<p>${escape(item.url)}</p>` : ""}</article>`).join("") ?? "";
      }
      if (content) {
        ...
        existing.forEach((node) => node.remove()); nodes.delete(entry.id);
      } else {
    ...
    for (const destination of profile.resumeManagerLayouts?.[templateId]?.length ? new Set([main, sidebar]) : []) {
